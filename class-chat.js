(function () {
  ...
  function initClassChat() {
    ...
    elements.gcAdminOpen?.addEventListener("click", () => openUtility("controls", { jumpToGcAdmin: true }));
  ...
  function openUtility(type, options = {}) {
    closeChatMenu();
    elements.utility.hidden = false;
    elements.searchPanel.hidden = type !== "search";
    elements.controlsForm.hidden = type !== "controls";
    if (elements.gcAdminPanel) elements.gcAdminPanel.hidden = currentProfile?.role !== "admin";
    ...
    if (type === "controls") {
      ...
      if (currentProfile?.role === "admin") {
        ...
        if (options.jumpToGcAdmin) {
          window.setTimeout(() => elements.gcAdminPanel?.scrollIntoView({ behavior: "smooth", block: "start" }), 120);
        }
      }
    ...
    if (type === "gc-admin") {
      openUtility("controls", { jumpToGcAdmin: true });
      return;
    }
    if (type === "poll") {
