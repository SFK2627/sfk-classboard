...
async function buildOfficerAttachmentPayload(inputId, notify) {
  ...
  try {
    return await Promise.all(files.map(file => readOfficerAttachmentFile(file)));
  } catch (error) {
...
async function readOfficerAttachmentFile(file) {
  const imageUrl = await readOfficerAttachmentDataUrl(file);
  const image = await loadOfficerAttachmentImage(imageUrl);
  const blob = await compressOfficerAttachmentImage(image);
  const dataUrl = await readOfficerAttachmentDataUrl(blob || file);
...
function readOfficerAttachmentDataUrl(fileOrBlob) {
...
function loadOfficerAttachmentImage(src) {
  return new Promise((resolve, reject) => {
...
async function compressOfficerAttachmentImage(image) {
