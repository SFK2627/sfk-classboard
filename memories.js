...
function getPhotoboothCanvasPlan(layout, theme = photoBoothState.theme) {
  ...
  if (theme === "feast-faustina") return getFeastFaustinaCanvasPlan(layout);
  if (layout === "grid4") {
...
function getFeastFaustinaCanvasPlan(layout) {
  ...
  return { width, height, footer, titleBlock, slots, saintBox };
}
...
async function renderPhotoboothCollage() {
  ...
  if (feast) {
    ...
    drawFeastFaustinaBanderitas(ctx, plan.width, 263, feastSpec);
